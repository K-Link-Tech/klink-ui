...
export function itSupportsInputProps<P>(
  ...
  it('handles error and invalid state', () => {
    ...
    expect(invalid.querySelectorAll(`.k-link-${name}-error`)).toHaveLength(0);
    expect(withError.querySelector(`.k-link-${name}-error`)).toBeInTheDocument();
    expect(withError.querySelector(`.k-link-${name}-error`).textContent).toBe('Test error');
  });
