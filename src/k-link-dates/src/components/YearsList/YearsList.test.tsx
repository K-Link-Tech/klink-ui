...
describe('@k-link/dates/YearsList', () => {
  itSupportsSystemProps({
    component: YearsList,
    props: defaultProps,
    refType: HTMLTableElement,
    providerName: 'YearsList',
    displayName: '@k-link/dates/YearsList',
  });
  ...
  it('has correct default __staticSelector', () => {
    ...
    expect(screen.getByRole('table')).toHaveClass('k-link-YearsList-yearsList');
    expect(screen.getAllByRole('button')[0]).toHaveClass('k-link-YearsList-pickerControl');
  });

  it('supports custom __staticSelector', () => {
    render(<YearsList {...defaultProps} __staticSelector="Calendar" />);
    expect(screen.getByRole('table')).toHaveClass('k-link-Calendar-yearsList');
