...
describe('@k-link/dates/MonthLevel', () => {
  itSupportsSystemProps({
    component: MonthLevel,
    props: defaultProps,
    refType: HTMLDivElement,
    providerName: 'MonthLevel',
    displayName: '@k-link/dates/MonthLevel',
  });
  ...
  it('has correct default __staticSelector', () => {
    ...
    expect(container.firstChild).toHaveClass('k-link-MonthLevel-monthLevel');
    expect(container.querySelector('table td button')).toHaveClass('k-link-MonthLevel-day');
    expect(screen.getByLabelText('level-control')).toHaveClass(
      'k-link-MonthLevel-calendarHeaderLevel'
    );
  });

  it('supports custom __staticSelector', () => {
    const { container } = render(<MonthLevel {...defaultProps} __staticSelector="Calendar" />);
    expect(container.firstChild).toHaveClass('k-link-Calendar-monthLevel');
    expect(container.querySelector('table td button')).toHaveClass('k-link-Calendar-day');
    expect(screen.getByLabelText('level-control')).toHaveClass(
