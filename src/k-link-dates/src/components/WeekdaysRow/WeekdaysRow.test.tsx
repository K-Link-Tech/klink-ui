...
describe('@k-link/dates/WeekdaysRow', () => {
  ...
  itSupportsSystemProps({
    ...
    displayName: '@k-link/dates/WeekdaysRow',
  ...
  it('has correct default __staticSelector', () => {
    render(<Wrapper />);
    expect(screen.getByRole('row')).toHaveClass('k-link-WeekdaysRow-weekdaysRow ');
  });

  it('supports __staticSelector', () => {
    render(<Wrapper __staticSelector="Month" />);
    expect(screen.getByRole('row')).toHaveClass(
      'k-link-WeekdaysRow-weekdaysRow',
      'k-link-Month-weekdaysRow '
    );
  });
