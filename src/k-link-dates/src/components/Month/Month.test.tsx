...
describe('@k-link/core/Month', () => {
  itSupportsSystemProps({
    component: Month,
    props: defaultProps,
    refType: HTMLTableElement,
    providerName: 'Month',
    displayName: '@k-link/dates/Month',
  });
  ...
  it('has correct default __staticSelector', () => {
    ...
    expect(container.querySelector('table')).toHaveClass('k-link-Month-month');
    expect(container.querySelector('thead tr')).toHaveClass('k-link-Month-weekdaysRow');
    expect(container.querySelector('tbody tr td button')).toHaveClass('k-link-Month-day');
  });

  it('supports __staticSelector', () => {
    const { container } = render(<Month {...defaultProps} __staticSelector="Calendar" />);
    expect(container.querySelector('table')).toHaveClass('k-link-Calendar-month');
    expect(container.querySelector('thead tr')).toHaveClass('k-link-Calendar-weekdaysRow');
    expect(container.querySelector('tbody tr td button')).toHaveClass('k-link-Calendar-day');
  });
