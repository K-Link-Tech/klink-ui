...
describe('@k-link/dates/MonthsList', () => {
  itSupportsSystemProps({
    component: MonthsList,
    props: defaultProps,
    refType: HTMLTableElement,
    providerName: 'MonthsList',
    displayName: '@k-link/dates/MonthsList',
  });
  ...
  it('has correct default __staticSelector', () => {
    ...
    expect(screen.getByRole('table')).toHaveClass('k-link-MonthsList-monthsList');
    expect(screen.getAllByRole('button')[0]).toHaveClass('k-link-MonthsList-pickerControl');
  });

  it('supports custom __staticSelector', () => {
    render(<MonthsList {...defaultProps} __staticSelector="Calendar" />);
    expect(screen.getByRole('table')).toHaveClass('k-link-Calendar-monthsList');
    expect(screen.getAllByRole('button')[0]).toHaveClass('k-link-Calendar-pickerControl');
  });
