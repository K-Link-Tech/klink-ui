...
describe('@k-link/dates/DecadeLevel', () => {
  itSupportsSystemProps({
    component: DecadeLevel,
    props: defaultProps,
    refType: HTMLDivElement,
    providerName: 'DecadeLevel',
    displayName: '@k-link/dates/DecadeLevel',
  });
  ...
  it('has correct default __staticSelector', () => {
    ...
    expect(container.firstChild).toHaveClass('k-link-DecadeLevel-decadeLevel');
    expect(container.querySelector('table td button')).toHaveClass(
      'k-link-DecadeLevel-pickerControl'
    );
    expect(screen.getByLabelText('level-control')).toHaveClass(
      'k-link-DecadeLevel-calendarHeaderLevel'
    );
  });

  it('supports custom __staticSelector', () => {
    const { container } = render(<DecadeLevel {...defaultProps} __staticSelector="Calendar" />);
    expect(container.firstChild).toHaveClass('k-link-Calendar-decadeLevel');
