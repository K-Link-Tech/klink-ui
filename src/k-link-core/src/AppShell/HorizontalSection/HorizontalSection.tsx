...
import { DefaultProps, MantineNumberSize, getDefaultZIndex, Global, rem, em } from '@k-link/styles';
import { Box } from '../../Box';
    ...
    const breakpoints = getSortedBreakpoints(width, theme).reduce(
      (acc, [breakpoint, breakpointSize]) => {
        acc[`@media (min-width: ${em(breakpoint)})`] = {
          [`--k-link-${section}-width`]: rem(breakpointSize),
        };
    ...
    return (
      ...
      >
        ...
        <Global
          styles={() => ({
            ':root': {
              [`--k-link-${section}-width`]: width?.base ? rem(width.base) : '0rem',
              ...breakpoints,
            },
          })}
        />
...
HorizontalSection.displayName = '@k-link/core/HorizontalSection';
