...
import { DefaultProps, getDefaultZIndex, Global, rem, em } from '@k-link/styles';
import { Box } from '../../Box';
      ...
      typeof height === 'object' && height !== null
        ? getSortedBreakpoints(height, theme).reduce((acc, [breakpoint, breakpointSize]) => {
            acc[`@media (min-width: ${em(breakpoint)})`] = {
              [`--k-link-${section}-height`]: rem(breakpointSize),
            };
...
VerticalSection.displayName = '@k-link/core/VerticalSection';
