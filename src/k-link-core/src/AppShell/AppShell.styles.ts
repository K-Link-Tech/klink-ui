...
function getPositionStyles(props: AppShellStylesParams, theme: MantineTheme): CSSObject {
  ...
  return {
    minHeight: '100vh',
    paddingTop: `calc(var(--k-link-header-height, 0px) + ${padding})`,
    paddingBottom: `calc(var(--k-link-footer-height, 0px) + ${padding})`,
    paddingLeft: `calc(var(--k-link-navbar-width, 0px) + ${padding})`,
    paddingRight: `calc(var(--k-link-aside-width, 0px) + ${padding})`,
