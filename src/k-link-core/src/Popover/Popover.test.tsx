...
import { itRendersChildren, checkAccessibility, wait, renderWithAct } from '@k-link/tests';
...
describe('@k-link/core/Popover', () => {
  ...
  it('correctly handles withArrow={true}', () => {
    ...
    expect(container.querySelectorAll('.k-link-Popover-arrow')).toHaveLength(1);
  });

  it('correctly handles withArrow={false}', () => {
    const { container } = render(<TestContainer defaultOpened withArrow={false} />);
    expect(container.querySelectorAll('.k-link-Popover-arrow')).toHaveLength(0);
  });
