...
describe('@k-link/core/InputLabel', () => {
  ...
  itSupportsSystemProps({
    ...
    props: defaultProps,
    displayName: '@k-link/core/InputLabel',
  ...
  it('renders required asterisk if required prop is true', () => {
    ...
    expect(document.querySelector('.k-link-InputWrapper-required')).toBeInTheDocument();
  });

  it('does not render required asterisk if required prop is false', () => {
    render(<InputLabel required={false} />);
    expect(document.querySelector('.k-link-InputWrapper-required')).not.toBeInTheDocument();
  });
});
