...
import { itSupportsRef, checkAccessibility } from '@k-link/tests';
...
describe('@k-link/core/Thumb', () => {
  ...
  it('shows label based on dragging and labelAlwaysOn props', () => {
    ...
    expect(labelAlwaysOn.querySelectorAll('.k-link-Slider-label')).toHaveLength(1);
    expect(dragging.querySelectorAll('.k-link-Slider-label')).toHaveLength(1);
    expect(labelOff.querySelectorAll('.k-link-Slider-label')).toHaveLength(0);
  });

  it('has correct displayName', () => {
    expect(Thumb.displayName).toStrictEqual('@k-link/core/SliderThumb');
  });
});
