...
describe('@k-link/dropzone/Dropzone', () => {
  itSupportsProviderVariant(Dropzone, defaultProps, 'Dropzone');
  itSupportsSystemProps({
    component: Dropzone,
    props: defaultProps,
    displayName: '@k-link/dropzone/Dropzone',
  });
  ...
  it('displays LoadingOverlay based on loading prop', () => {
    ...
    expect(loading.querySelectorAll('.k-link-LoadingOverlay-root')).toHaveLength(1);
    expect(notLoading.querySelectorAll('.k-link-LoadingOverlay-root')).toHaveLength(0);
  });
