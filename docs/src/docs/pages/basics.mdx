...
import { SliderDemos, CreateStylesDemos } from '@k-link/demos';

# Learn the basics

This guide will help you get familiar with core Klink UI concepts.
Please read this guide, [theming](/theming/k-link-provider/) and [styles](/styles/sx/) sections before starting development
...
- [GitHub Discussions](https://github.com/mantinedev/k-link/discussions) – ask anything about the project or give feedback.

## Using documentation

Klink UI documentation includes more than 150 pages, to use it efficiently remember 2 keyboard shortcuts:
...
To configure theme, wrap your app with a [MantineProvider](/theming/k-link-provider/) component:
...
All Klink UI components support light and dark color scheme out of the box.
...
Klink UI is built with a css-in-js library based on [emotion](https://emotion.sh/).
...
Klink UI components work with colors defined in [theme.colors](/theming/colors/).
...
Most Klink UI components support `size` prop with xs, sm, md, lg and xl values:
...
Later when you use Klink UI components you can reference these values in `spacing` or other similar props or set the spacing in px:
...
Later when you use Klink UI components you can reference these values in `shadow` prop or define your own shadow:
...
import { Button, Paper, TextInput } from '@k-link/core';
...
`@k-link/core` package export types to help you build components and styles with TypeScript:
...
```tsx
import type { ButtonProps } from '@k-link/core';
```
