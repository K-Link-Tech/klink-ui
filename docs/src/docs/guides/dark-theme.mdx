...
You can create these styles on your own or add them by setting `withGlobalStyles` prop on [MantineProvider](/theming/klink-ui-provider/), which includes them by default:
...
import { MantineProvider, ColorSchemeProvider, ColorScheme } from '@klink-ui/core';
...
import { MantineProvider, ColorScheme, ColorSchemeProvider } from '@klink-ui/core';
...
export default function App(props: AppProps & { colorScheme: ColorScheme }) {
  ...
  const toggleColorScheme = (value?: ColorScheme) => {
    ...
    setCookie('klink-ui-color-scheme', nextColorScheme, { maxAge: 60 * 60 * 24 * 30 });
  };
...
App.getInitialProps = async (appContext: AppContext) => {
  ...
  return {
    ...appProps,
    colorScheme: getCookie('klink-ui-color-scheme', appContext.ctx) || 'light',
  };
};
```

Full code is available in [Klink UI Next.js template](https://github.com/mantinedev/klink-ui-next-template).
