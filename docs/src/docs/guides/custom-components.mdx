...
This guide will help you learn how to integrate custom components with [MantineProvider](/theming/k-link-provider/)
...
import { createStyles, MantineNumberSize } from '@k-link/core';
...
```tsx
// MyComponent.tsx
import { DefaultProps, Selectors, MantineNumberSize, Box } from '@k-link/core';
...
You can also add `styles` and `classNames` on [MantineProvider](/theming/k-link-provider/):
...
Then you will be able to add [default props](/theming/default-props/) on [MantineProvider](/theming/k-link-provider/):
...
```tsx
import { DefaultProps } from '@k-link/core';
