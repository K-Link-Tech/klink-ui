...
Klink UI is built with [emotion](https://emotion.sh) which supports RTL languages with [stylis-plugin-rtl](https://github.com/styled-components/stylis-plugin-rtl).
...
Add `stylis-plugin-rtl` and `theme.dir` to your [MantineProvider](/theming/klink-ui-provider/):
...
import { MantineProvider, Button, createEmotionCache } from '@klink-ui/core';
import rtlPlugin from 'stylis-plugin-rtl';
