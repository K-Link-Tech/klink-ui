...
The easiest way to get started is to use [default](https://github.com/mantinedev/klink-ui-next-template) or [minimal template](https://github.com/mantinedev/klink-ui-minimal-next-template) on GitHub:

1. Open [default template](https://github.com/mantinedev/klink-ui-next-template) or [minimal template](https://github.com/mantinedev/klink-ui-minimal-next-template)
...
<PackagesInstallation extraPackages={['@klink-ui/next', '@emotion/server']} />

Create `pages/_document.tsx` file:

```tsx
import { createGetInitialProps } from '@klink-ui/next';
...
Add [MantineProvider](/theming/klink-ui-provider/) to `pages/_app.tsx`:
...
for that you will need to setup Klink UI ssr styles extraction on your side:
