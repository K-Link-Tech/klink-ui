...
import: "import { Accordion } from '@k-link/core';"
source: 'k-link-core/src/Accordion/Accordion.tsx'
docs: 'core/Accordion.mdx'
styles: ['Accordion']
---

import { AccordionDemos } from '@k-link/demos';
...
```tsx
import { Accordion, rem } from '@k-link/core';
...
`AccordionProps` type exported from `@k-link/core` is a generic, it accepts boolean type that
describes `multiple` state:

```tsx
import type { AccordionProps } from '@k-link/core';
