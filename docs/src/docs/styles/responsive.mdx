...
import { MediaQueryDemos, CreateStylesDemos } from '@klink-ui/demos';

# Responsive styles

`@klink-ui/core` package exports several components that can help you organize responsive styles without writing CSS.
...
`theme.breakpoints` are used in all responsive Klink UI components. Breakpoints are expected to be set in `em` units (`1em == 16px` by default).
You can configure these values with [MantineProvider](/theming/klink-ui-provider/):
...
Most of Klink UI components support `size` prop with `xs`, `sm`, `md`, `lg` and `xl` values.
...
All Klink UI components support `sx` prop with which you can add styles (including responsive styles) to root component element:
...
If you want to add styles with `sx` prop to non-Klink UI component, you can wrap it in [Box](/core/box/):
