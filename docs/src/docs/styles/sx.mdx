...
All Klink UI components support `sx` prop. With `sx` you can add styles to component **root** element.
...
```tsx
import { Button, Sx } from '@klink-ui/core';
...
function MyButton({ sx }: MyButtonProps) {
  return <Button sx={[{ color: 'error' }, sx]}>My Button</Button>;
}
```
...
```tsx
import { Button, packSx, Sx } from '@klink-ui/core';
...
function MyButton({ sx }: MyButtonProps) {
  return <Button sx={[{ color: 'error' }, ...packSx(sx)]}>My Button</Button>;
}
```
...
```tsx
import { Button, packSx, DefaultProps } from '@klink-ui/core';

interface MyButtonProps extends DefaultProps {}

function MyButton({ sx, ...others }: MyButtonProps) {
  return (
    <Button sx={[{ color: 'error' }, ...packSx(sx)]} {...others}>
      My Button
    </Button>
  );
}
...
that can be used to style non Klink UI components or elements with `sx` prop:
...
```tsx
import { Text, Box, CSSObject, rem } from '@klink-ui/core';

// styles object is compatible with every Klink UI component
...
```tsx
import { Text, Box, CSSObject, MantineTheme, rem } from '@klink-ui/core';
