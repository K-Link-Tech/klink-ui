...
import { CreateStylesDemos } from '@klink-ui/demos';

# createStyles

All Klink UI components are built with [emotion](https://emotion.sh/) based css-in-js library.
We recommend using `createStyles` to create styles for the rest of your application
as it provides the most convenient way to utilize Klink UI theme, but it is not required
...
defined with [MantineProvider](/theming/klink-ui-provider) or just static values:
