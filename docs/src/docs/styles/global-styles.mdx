...
Usually global styles depend on theme, you can render `Global` component anywhere within [MantineProvider](/theming/klink-ui-provider/):

```tsx
import { Global, MantineProvider } from '@klink-ui/core';
...
To load use array syntax. The following example shows how Klink UI docs website loads GreycliffCF font:
