...
slug: /styles/rem/
order: 1
search: 'Learn how to use rem units with Klink UI components'
---
...
All Klink UI components use `rem` units to apply size styles. `1rem` is considered to be `16px`
...
You can use numbers in Klink UI components props, it will be treated as `px` and converted to `rem`,
for example:
...
`@klink-ui/core` package exports `rem` and `em` function that can be used to convert `px` into `rem`/`em`:

```tsx
import { rem, em } from '@klink-ui/core';
...
To convert `rem` to `px` use `px` function exported from `@klink-ui/core`:

```tsx
import { px } from '@klink-ui/core';
