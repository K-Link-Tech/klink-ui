...
`@k-link/core` package exports `rem` and `em` function that can be used to convert `px` into `rem`/`em`:

```tsx
import { rem, em } from '@k-link/core';
...
To convert `rem` to `px` use `px` function exported from `@k-link/core`:

```tsx
import { px } from '@k-link/core';
