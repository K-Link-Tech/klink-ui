...
import { Slider as SliderStylesApi, Button as ButtonStylesApi } from '@klink-ui/styles-api';

# Styles API

With Styles API you can overwrite styles of inner elements in Klink UI components with `classNames` or
`styles` props.
...
<Slider styles={{ thumb: { backgroundColor: 'error' } }} />
```
...
```tsx
import { Slider, createStyles, rem } from '@klink-ui/core';
...
All Klink UI components support `unstyled` prop which removes all non-essential Klink UI styles
...
Unstyled [Tabs](https://klink-ui.dev/core/tabs/):

<Demo data={TabsDemos.unstyled} demoProps={{ toggle: true }} />

Unstyled [Accordion](https://klink-ui.dev/core/accordion/):
