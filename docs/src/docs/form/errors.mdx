...
`form.errors` type is `Record<string, React.ReactNode>`, you can import a shorthand `FormErrors` type from `@klink-ui/form`:

```tsx
import type { FormErrors } from '@klink-ui/form';
```
