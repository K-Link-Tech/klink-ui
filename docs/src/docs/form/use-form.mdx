...
import: "import { useForm } from '@k-link/form';"
...
`@k-link/form` package does not depend on any other libraries, you can use it with or without `@k-link/core` inputs:
...
import { useForm, UseFormReturnType } from '@k-link/form';
