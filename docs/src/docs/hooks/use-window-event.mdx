...
import: "import { useWindowEvent } from '@klink-ui/hooks';"
docs: 'hooks/use-window-event.mdx'
source: 'klink-ui-hooks/src/use-window-event/use-window-event.ts'
---
...
import { useWindowEvent } from '@klink-ui/hooks';
...
Search focus with `⌘ + K` on mac or `Ctrl + K` on windows and linux on Klink UI docs website:
