...
import: "import { useLocalStorage } from '@klink-ui/hooks';"
docs: 'hooks/use-local-storage.mdx'
source: 'klink-ui-hooks/src/use-local-storage/use-local-storage.ts'
---
...
```tsx
import { useLocalStorage } from '@klink-ui/hooks';
...
Klink UI docs website uses this hook to store color scheme information:

```tsx
import { useLocalStorage } from '@klink-ui/hooks';
import { ActionIcon, ColorScheme } from '@klink-ui/core';
...
You can test this feature by opening 2 tabs with Klink UI docs side by side and changing color scheme
...
```tsx
import { useSessionStorage } from '@klink-ui/hooks';
