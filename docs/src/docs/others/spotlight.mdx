...
import: "import { SpotlightProvider } from '@k-link/spotlight';"
source: 'k-link-spotlight/src'
docs: 'others/spotlight.mdx'
installation: '@k-link/spotlight'
license: MIT
styles: ['SpotlightProvider']
---

import { SpotlightDemos } from '@k-link/demos';
...
```bash
yarn add @k-link/spotlight
```

Install with npm:

```bash
npm install @k-link/spotlight
```
...
Note that if you are using [MantineProvider](/theming/k-link-provider/), `SpotlightProvider` must be used as its child:
...
`@k-link/spotlight` exports `spotlight` object with functions which can be used to perform certain actions
from any part of your application:

```tsx
// All functions can be called from anywhere (not only from components)
import { spotlight } from '@k-link/spotlight';
...
You can import `SpotlightAction` type from `@k-link/spotlight` package:
...
```tsx
import { RemoveScroll } from '@k-link/core';
