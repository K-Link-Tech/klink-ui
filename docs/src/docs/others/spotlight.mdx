...
import: "import { SpotlightProvider } from '@klink-ui/spotlight';"
source: 'klink-ui-spotlight/src'
docs: 'others/spotlight.mdx'
installation: '@klink-ui/spotlight'
license: MIT
styles: ['SpotlightProvider']
---

import { SpotlightDemos } from '@klink-ui/demos';
...
```bash
yarn add @klink-ui/spotlight
```

Install with npm:

```bash
npm install @klink-ui/spotlight
```
...
Note that if you are using [MantineProvider](/theming/klink-ui-provider/), `SpotlightProvider` must be used as its child:
...
You can setup multiple shortcuts, for example, Klink UI documentation uses the following setup:
...
`@klink-ui/spotlight` exports `spotlight` object with functions which can be used to perform certain actions
from any part of your application:

```tsx
// All functions can be called from anywhere (not only from components)
import { spotlight } from '@klink-ui/spotlight';
...
You can import `SpotlightAction` type from `@klink-ui/spotlight` package:
...
```tsx
import { RemoveScroll } from '@klink-ui/core';
