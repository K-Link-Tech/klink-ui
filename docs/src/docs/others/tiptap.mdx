...
import: "import { RichTextEditor } from '@klink-ui/tiptap';"
source: 'klink-ui-tiptap/src'
docs: 'others/tiptap.mdx'
installation: '@klink-ui/tiptap'
license: MIT
...
```bash
yarn add @klink-ui/tiptap @klink-ui/core @klink-ui/hooks @tabler/icons-react @tiptap/react @tiptap/extension-link @tiptap/starter-kit
```

Install with npm:

```bash
npm install @klink-ui/tiptap @klink-ui/core @klink-ui/hooks @tabler/icons-react @tiptap/react @tiptap/extension-link @tiptap/starter-kit
```

## TipTap editor

`@klink-ui/tiptap` provides a UI for [Tiptap](https://tiptap.dev/). `RichTextEditor` component
...
`@klink-ui/tiptap` provides custom `Link` extension that is required to be used instead of
`@tiptap/extension-link` in order for `Ctrl + K` keyboard shortcut to work:

```tsx
// Use Link extension exported from the @klink-ui/tiptap package
...
```tsx
import { useRichTextEditorContext } from '@klink-ui/tiptap';
...
```tsx
// RichTextEditorLabels type can be imported from @klink-ui/tiptap package
...
Default labels (can be imported from `@klink-ui/tiptap` package):
