...
import: "import { RichTextEditor } from '@k-link/tiptap';"
source: 'k-link-tiptap/src'
docs: 'others/tiptap.mdx'
installation: '@k-link/tiptap'
license: MIT
...
```bash
yarn add @k-link/tiptap @k-link/core @k-link/hooks @tabler/icons-react @tiptap/react @tiptap/extension-link @tiptap/starter-kit
```

Install with npm:

```bash
npm install @k-link/tiptap @k-link/core @k-link/hooks @tabler/icons-react @tiptap/react @tiptap/extension-link @tiptap/starter-kit
```

## TipTap editor

`@k-link/tiptap` provides a UI for [Tiptap](https://tiptap.dev/). `RichTextEditor` component
...
`@k-link/tiptap` provides custom `Link` extension that is required to be used instead of
`@tiptap/extension-link` in order for `Ctrl + K` keyboard shortcut to work:

```tsx
// Use Link extension exported from the @k-link/tiptap package
...
website there is a header with `60px` height (converted to rem), in this case we will need to
...
```tsx
import { useRichTextEditorContext } from '@k-link/tiptap';
...
```tsx
// RichTextEditorLabels type can be imported from @k-link/tiptap package
...
Default labels (can be imported from `@k-link/tiptap` package):
