...
props: ['Notifications']
source: 'klink-ui-notifications/src'
docs: 'others/notifications.mdx'
pageTitle: 'Klink UI Notifications'
description: 'Klink UI notifications system'
installation: '@klink-ui/notifications'
license: MIT
---

import { NotificationDemos, NotificationsDemos } from '@klink-ui/demos';
...
```bash
yarn add @klink-ui/notifications
```

Install with npm:

```bash
npm install @klink-ui/notifications
```
...
inside [MantineProvider](/theming/klink-ui-provider/) next to your `App` component:
...
`@klink-ui/notifications` package exports the following functions:
...
All functions can be imported from `@klink-ui/notifications` package and can be used in any part of your application:
