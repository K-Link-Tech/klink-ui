...
import: "import { Dropzone } from '@klink-ui/dropzone';"
source: 'klink-ui-dropzone/src'
docs: 'others/dropzone.mdx'
installation: '@klink-ui/dropzone'
license: MIT
...
} from '@klink-ui/dropzone';
import { DropzoneDemos } from '@klink-ui/demos';
...
```bash
yarn add @klink-ui/dropzone
```

Install with npm:

```bash
npm install @klink-ui/dropzone
```
...
To save some research time you can use `MIME_TYPES` variable exported from `@klink-ui/dropzone`:

```tsx
import { Dropzone, MIME_TYPES } from '@klink-ui/dropzone';
...
```tsx
import { IMAGE_MIME_TYPE, Dropzone } from '@klink-ui/dropzone';
