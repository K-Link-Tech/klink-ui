...
import: "import { Dropzone } from '@k-link/dropzone';"
source: 'k-link-dropzone/src'
docs: 'others/dropzone.mdx'
installation: '@k-link/dropzone'
license: MIT
...
} from '@k-link/dropzone';
import { DropzoneDemos } from '@k-link/demos';
...
```bash
yarn add @k-link/dropzone
```

Install with npm:

```bash
npm install @k-link/dropzone
```
...
To save some research time you can use `MIME_TYPES` variable exported from `@k-link/dropzone`:

```tsx
import { Dropzone, MIME_TYPES } from '@k-link/dropzone';
...
```tsx
import { IMAGE_MIME_TYPE, Dropzone } from '@k-link/dropzone';
