...
Klink UI uses [open-color](https://yeun.github.io/open-color/) in default theme with some additions.
...
When you use `color` prop in any Klink UI component, it will reference `theme.primaryShade` to get
...
TypeScript will only autocomplete Klink UI's default colors when accessing the theme. To add your custom colors to the MantineColor type, you can use TypeScript module declaration.

```ts
import { Tuple, DefaultMantineColor } from '@klink-ui/core';
