...
slug: /theming/klink-ui-provider/
order: 1
...
```css
.my-button {
  background-color: var(--klink-ui-color-blue-6);
  font-family: var(--klink-ui-font-family);
  line-height: var(--klink-ui-line-height);
}
```

Mantine exposes the following CSS variables based on [theme](/theming/theme-object/) you provide:

- `--klink-ui-color-white`
- `--klink-ui-color-black`
- `--klink-ui-transition-timing-function`
- `--klink-ui-line-height`
- `--klink-ui-font-family`
- `--klink-ui-font-family-monospace`
- `--klink-ui-font-family-headings`
- `--klink-ui-heading-font-weight`
- `--klink-ui-shadow-{size}`, e.g. `--klink-ui-shadow-sm`, `--klink-ui-shadow-xl`
- `--klink-ui-radius-{size}`, e.g. `--klink-ui-radius-sm`, `--klink-ui-radius-xl`
- `--klink-ui-spacing-{size}`, e.g. `--klink-ui-spacing-sm`, `--klink-ui-spacing-xl`
- `--klink-ui-font-size-{size}`, e.g. `--klink-ui-font-size-sm`, `--klink-ui-font-size-xl`
- `--klink-ui-color-{color}-{shade}`, e.g. `--klink-ui-color-blue-6`, `--klink-ui-color-gray-0`
- `--klink-ui-{heading}-font-size`, e.g. `--klink-ui-h1-font-size`
- `--klink-ui-{heading}-line-height`, e.g, `--klink-ui-h3-line-height`
...
```tsx
import { MantineProvider, Text } from '@klink-ui/core';
