...
order: 0
search: 'Theme object, use-klink-ui-theme, theme overrides, MantineTheme type'
---

import { ThemeDemos } from '@klink-ui/demos';

# Theme object

Klink UI theme is an object where your application's colors, fonts, spacing, border-radius and other design tokens are stored.
...
interface MantineTheme {
  ...
  // Add your own custom properties on Klink UI theme
...
To customize theme, pass theme override object to [MantineProvider](/theming/klink-ui-provider/) `theme` prop.
Theme override will be deeply merged with [default theme](https://github.com/mantinedev/klink-ui/blob/master/src/klink-ui-styles/src/theme/default-theme.ts).
...
`theme.defaultRadius` controls default border-radius of all Klink UI components ([Button](/core/button/), [TextInput](/core/text-input/), [ThemeIcon](/core/theme-icon/), etc.), default value is `sm`.
...
Klink UI theme has the following properties associated with colors:
...
```tsx
import { MantineThemeOverride, MantineProvider } from '@klink-ui/core';
...
## use-klink-ui-theme hook

`use-klink-ui-theme` hook returns theme from [MantineProvider](/theming/klink-ui-provider/) context or default theme
...
You can import `MantineTheme` type from `@klink-ui/core` package:

```tsx
import { useMantineTheme, MantineTheme } from '@klink-ui/core';
