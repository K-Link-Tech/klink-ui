...
order: 0
search: 'Theme object, use-k-link-theme, theme overrides, MantineTheme type'
---
...
To customize theme, pass theme override object to [MantineProvider](/theming/k-link-provider/) `theme` prop.
Theme override will be deeply merged with [default theme](https://github.com/mantinedev/k-link/blob/master/src/k-link-styles/src/theme/default-theme.ts).
...
```tsx
import { MantineThemeOverride, MantineProvider } from '@k-link/core';
...
## use-k-link-theme hook

`use-k-link-theme` hook returns theme from [MantineProvider](/theming/k-link-provider/) context or default theme
...
You can import `MantineTheme` type from `@k-link/core` package:

```tsx
import { useMantineTheme, MantineTheme } from '@k-link/core';
