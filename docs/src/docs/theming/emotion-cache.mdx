...
All Klink UI components are styled with [emotion](https://emotion.sh/).
In some cases you will need to create custom emotion cache, for example to
handle RTL direction.

## Configure emotion

By default, Klink UI create [emotion cache](https://emotion.sh/docs/@emotion/cache) for you automatically.
...
By default, Klink UI components styles are prepended to head to allow overrides.
To make Klink UI styles override other styles, create emotion cache with `prepend: false`.
...
the same emotion cache both to [MantineProvider](/theming/klink-ui-provider/)
and to the server side code.

Example with Next.js ([view full code on GitHub](https://github.com/mantinedev/klink-ui-next-template/tree/v5-rtl)):

```tsx
// rtl-cache.ts
import rtlPlugin from 'stylis-plugin-rtl';
import { createEmotionCache } from '@klink-ui/core';
...
import { ServerStyles, createStylesServer } from '@klink-ui/next';
import { rtlCache } from '../rtl-cache';
