...
Klink UI theme provides several functions that can be used to simplify writing styles.
All functions available at `theme.fn` object, for example `theme.fn.cover()`.

## Accessing theme functions

You can use theme functions everywhere Klink UI [theme](/theming/theme-object/) is available.
...
function Demo() {
  return (
    <Box
      sx={(theme) => ({
        [theme.fn.largerThan('md')]: { color: 'error' },
      })}
    />
  );
}
...
function Demo() {
  return (
    <Slider
      styles={(theme) => ({
        track: {
          background: theme.fn.linearGradient(45, 'error', 'blue'),
        },
      })}
    />
  );
...
function Demo() {
  ...
  return <div style={{ background: theme.fn.linearGradient(45, 'error', 'blue') }} />;
}
```

## focusStyles

All Klink UI components have custom focus ring styles which are based on [:focus-visible](https://developer.mozilla.org/en-US/docs/Web/CSS/:focus-visible)
pseudo-class. It is important to keep styles consistent across all of your application components and you should
include Klink UI focus styles to your custom interactive elements. `theme.fn.focusStyles` function lets you spread Klink UI focus styles
...
With `theme.fn.fontStyles` function you can add font styles from your Klink UI theme to any element.
...
theme.fn.linearGradient(133, 'blue', 'error', 'orange', 'blue_light', 'white');
...
theme.fn.radialGradient('blue', 'error', 'orange', 'blue_light', 'white');
...
theme.colors.error[theme.fn.primaryShade()];
```
