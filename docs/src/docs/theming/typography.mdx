...
`theme.fontSizes` property defines font-size values for all Klink UI components:
...
By default, Klink UI uses system fonts. It means that different devices will display components based on available font,
...
[MantineProvider](/theming/klink-ui-provider/):
...
The following example shows how Klink UI docs website loads `Greycliff CF` font:
...
Then you will be able to use custom fonts in any component or [MantineProvider](/theming/klink-ui-provider/):
