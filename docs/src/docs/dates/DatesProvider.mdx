...
description: 'Settings provider for @k-link/dates components'
import: "import { DatesProvider } from '@k-link/dates';"
source: 'k-link-dates/src/components/DatesProvider/DatesProvider.tsx'
docs: 'dates/DatesProvider.mdx'
---

import { DatesProviderDemos } from '@k-link/demos';

## Usage

`DatesProvider` component lets you set various settings that are shared across all
components exported from `@k-link/dates` package. `DatesProvider` supports the following settings:
