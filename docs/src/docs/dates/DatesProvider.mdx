---
group: 'klink-ui-dates'
package: '@klink-ui/dates'
title: DatesProvider
order: 0
slug: /dates/dates-provider/
description: 'Settings provider for @klink-ui/dates components'
import: "import { DatesProvider } from '@klink-ui/dates';"
source: 'klink-ui-dates/src/components/DatesProvider/DatesProvider.tsx'
docs: 'dates/DatesProvider.mdx'
---

import { DatesProviderDemos } from '@klink-ui/demos';

## Usage

`DatesProvider` component lets you set various settings that are shared across all
components exported from `@klink-ui/dates` package. `DatesProvider` supports the following settings:
